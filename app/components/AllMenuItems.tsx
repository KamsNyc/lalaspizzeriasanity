...
export default function AllMenuItems({ category }: { category: string }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const filteredItems = category && category !== 'All' ? menuItems.filter(item => item.categories && item.categories.some(cat => cat.title === category)
              )
            : menuItems;
  ...
  return (
    <div className='flex flex-col items-center md:mt-[32px]'>
